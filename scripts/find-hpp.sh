...
# Cross-platform script to find `.hpp` (header) files
# Works on both Linux (GNU find) and macOS (BSD find)
# EXAMPLE: To search for `class HashTable`, run as `./find-hpp.sh hash table`.
...
if [[ $# -gt 0 ]]; then
	...
	# e.g., for "hash table", find: hash_table.h, hash-table.hpp, hashtable.hh, etc.
	class_pattern=""
	for arg in "$@"; do
	...
	# If no arguments, find all .h/.hpp/.hh files
	for ext in "${extensions[@]}"; do
		patterns+=("*.${ext}")
	done
fi
...
if [[ "$OS" == "macos" ]]; then
	...
	# Linux (GNU find): use multiple -iname with OR
	find_args=("-type" "f" "(" )
	for i in "${!patterns[@]}"; do
		find_args+=("-iname" "${patterns[$i]}")
		if [[ $((i + 1)) -lt ${#patterns[@]} ]]; then
