...
class SubmissionImporter:
        ...
        if not nested:
            if self.required_filename:
                ...
                for index, path in enumerate(paths):
                    member = str(original_paths[index])
                    key = self._submission_group_key(path.name)
                    if key not in grouped:
                        grouped[key] = (path.name, [])
                    grouped[key][1].append(member)
    ...
    def _submission_group_key(self, filename: str) -> str:
        ...
        for known_name in known_names:
            ...
            match = re.search(
                rf"(?:^|[-_ ]){re.escape(known_stem)}(?:-\d+)?$",
                stem,
                flags=re.IGNORECASE,
            )
            if match:
                prefix = stem[: match.start()].rstrip("-_ ")
                # Canvas uses the first numeric metadata value for the
                # submission and gives each uploaded file its own second
                # numeric value. Remove only that final numeric component
                # from the metadata prefix when grouping files.
                prefix = re.sub(r"[-_]\d+$", "", prefix)
                return prefix or "submission"
        return stem
